...
Сайт Telegram‑канала [@forgottenphotocards](https://t.me/forgottenphotocards) — backend на FastAPI, который сам тащит посты и фотографии из публичной превью‑страницы канала, и статический SPA‑фронт.

## Стек

- **Backend** — Python 3.13, FastAPI, SQLAlchemy 2 (async) + SQLite (`aiosqlite`), APScheduler. Управление зависимостями — `uv`.
- **Scraper** — `httpx` + `selectolax`, парсит `https://t.me/s/<channel>` (без Telegram API).
- **Frontend** — React 18 как `<script>`‑бандл, JSX транспилируется esbuild’ом на этапе сборки контейнера. Без Vite/Webpack/npm‑install в проде.
- **Reverse‑proxy** — отдельный nginx перед фронтом и бэком, в проде с TLS.
...
Параллельная отдельностоящая версия фронта (без бэка, с моками) — для интеграции в чужой проект как готового SPA. К текущему стеку не относится, оставлено как референс и для демо‑превью.
